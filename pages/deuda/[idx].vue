...
const deudaId = route.params.idx

// Estado
const { items: deudas, updateItem, fetchItems } = useFirestore('deudas')
const deuda = computed(() => deudas.value.find(d => d.id === deudaId))
...
async function registrarPago() {
  ...
  const historialActual = deuda.value.historialPagos || []

  // Calcular el nuevo saldo restante
  const totalPagadoAntes = historialActual.reduce((sum, pago) => sum + pago.monto, 0)
  const saldoRestante = deuda.value.montoTotal - (totalPagadoAntes + formPago.value.monto)
  ...
  const totalPagado = totalPagadoAntes + formPago.value.monto
  const pagosRealizados = Math.min(
    Math.ceil(totalPagado / deuda.value.cuotaMensual),
    deuda.value.plazoMeses
  )

  // Actualizar en Firestore
  const result = await updateItem(deuda.value.id, {
    historialPagos: nuevoHistorial,
    pagosRealizados,
    fechaUltimoPago: formPago.value.fecha
  })
